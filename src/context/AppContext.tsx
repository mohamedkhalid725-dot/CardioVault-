...
export const AppProvider:React.FC<{children:React.ReactNode}>=({children})=>{
 ...
 const loginWithGoogle=async()=>{
   ...
   try{
     ...
     const profile=AuthorizationService.resolveUserForFirebaseAuth(user);setCurrentUserState(profile);const a={...auth,isAuthenticated:true,isLocked:false,pinCode:'',userEmail:user.email||auth.userEmail,userName:user.displayName||auth.userName};
     setAuth(a);
 ...
 const addBed=(unitId:string,bedNumber?:string)=>{const n=beds.filter(b=>b.unitId===unitId).length+1;const bed={id:`bed-${unitId}-${Date.now()}`,unitId,bedNumber:bedNumber||`Bed ${n}`,status:'Empty' as PatientStatus};commitBeds([...beds,bed]);commitUnits(units.map(u=>u.id===unitId?{...u,totalBeds:u.totalBeds+1}:u));void notifyClinicalData(`تم إضافة سرير جديد: ${bed.bedNumber}.`,'تمت إضافة بيانات جديدة');return bed;};
