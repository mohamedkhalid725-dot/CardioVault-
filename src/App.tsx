...
const AppContent:React.FC=()=>{
 ...
 const[accessReady,setAccessReady]=useState(()=>!Capacitor.isNativePlatform()||!!getStoredWorkspaceAccess());
 const handlingPop=useRef(false);
 const hasLocalSession=!!localStorage.getItem('cardiovault_auth_v2');
 useEffect(()=>{if(!auth.isAuthenticated||auth.isLocked||!hasLocalSession||!Capacitor.isNativePlatform())return;let attempts=0;const check=()=>{const ready=!!getStoredWorkspaceAccess();setAccessReady(ready);if(!ready&&attempts<16){attempts+=1;window.setTimeout(check,500);}};check();const onAccess=()=>setAccessReady(true);window.addEventListener('cardiovault-workspace-access-granted',onAccess);window.addEventListener('cardiovault-data-restored',onAccess);return()=>{window.removeEventListener('cardiovault-workspace-access-granted',onAccess);window.removeEventListener('cardiovault-data-restored',onAccess);};},[auth.isAuthenticated,auth.isLocked,hasLocalSession]);
 ...
 if(Capacitor.isNativePlatform()&&!accessReady)return <UnitAccessGate/>;
