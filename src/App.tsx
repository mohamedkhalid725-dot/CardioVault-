...
const AppContent:React.FC=()=>{
 ...
 useEffect(()=>{
   if(!auth.isAuthenticated||auth.isLocked||!hasLocalSession)return;
   if(!window.history.state?.cardiovault){window.history.replaceState({cardiovault:true,view:currentView},'');}
 ...
 useEffect(()=>{
   ...
   if(current!==currentView)window.history.pushState({cardiovault:true,view:currentView},'');
 },[currentView,auth.isAuthenticated,auth.isLocked,hasLocalSession]);
 useEffect(()=>{
   if(!auth.isAuthenticated||auth.isLocked||!hasLocalSession)return;
   const onPop=()=>{
     const view=window.history.state?.view;
     if(view){handlingPop.current=true;setCurrentView(view);}
   };
   const onBack=async()=>{
     if(currentView==='patient'){
       const detail={handled:false};
       window.dispatchEvent(new CustomEvent('cardiovault-patient-back',{detail}));
   ...
   CapacitorApp.addListener('backButton',onBack).then(x=>{listener=x;}).catch(()=>{});
   return()=>{window.removeEventListener('popstate',onPop);void listener?.remove();};
 },[auth.isAuthenticated,auth.isLocked,hasLocalSession,setCurrentView,currentView]);
 if(!auth.isAuthenticated||auth.isLocked||!hasLocalSession)return <LoginScreen/>;
 const render=()=>{switch(currentView){case'home':return <UnitsDashboard/>;case'census':return <UnitCensusView/>;case'handover':return <HandoverView/>;case'patient':return <PatientFileView/>;case'patients':return <ActivePatientsView/>;case'add-patient':return <AddPatientPage/>;case'archive':return <ArchiveView/>;case'calculators':return <CalculatorsHub/>;case'settings':return <SettingsView/>;default:return <UnitsDashboard/>;}};
