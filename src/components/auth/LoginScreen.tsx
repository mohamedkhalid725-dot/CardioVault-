...
export const LoginScreen: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      localStorage.setItem('cardiovault_google_uid', googleUser.uid);
      ...
      try {
        ...
        showToast('Google sign-in succeeded. Cloud sync will retry from the Sync button.', 'warning');
      }
      showToast('Signed in with Google and Cloud Sync enabled.', 'success');
    } catch (error: any) {
      console.error('Native Google Sign-In error:', error);
      const message = String(error?.message || error?.code || 'Unknown error');
