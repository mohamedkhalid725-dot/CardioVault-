    ...
    <div className="relative h-[4.05rem]">
     <div className={`absolute inset-x-0 top-0 flex items-center gap-2.5 min-h-[4.05rem] ${compact?'opacity-100 translate-y-0':'opacity-100'}`}>
      ...
      <div className="min-w-0 flex-1">
       <div className="flex items-center gap-2 min-w-0"><h1 className={`font-extrabold truncate ${compact?'text-sm':'text-lg sm:text-xl'}`}>{patient.fullName}</h1>{readOnly?<span className="text-[9px] font-bold px-2 py-1 rounded-full bg-amber-500/15 text-amber-600 shrink-0">READ ONLY</span>:<div className="relative shrink-0"><select value={patient.status} onChange={e=>handleStatusChange(e.target.value as PatientStatus)} className={`appearance-none text-[10px] sm:text-xs font-bold px-2.5 py-1 rounded-full border pr-6 ${patient.status==='Critical'?'bg-rose-500/15 text-rose-500 border-rose-500/40':patient.status==='Unstable'?'bg-amber-500/15 text-amber-500 border-amber-500/40':'bg-emerald-500/15 text-emerald-500 border-emerald-500/40'}`}><option>Critical</option><option>Unstable</option><option>Stable</option></select><ChevronDown className="absolute right-1.5 top-1/2 -translate-y-1/2 w-3 h-3 pointer-events-none"/></div>}</div>
