...
export const PatientFileView:React.FC=()=>{
 ...
 const[sectionHistory,setSectionHistory]=useState([activePatientSection]);
 ...
 useEffect(()=>{
   const onBack=(event:Event)=>{
     ...
     if(sectionOpen&&sectionHistory.length>1){
       const next=sectionHistory[sectionHistory.length-2];
       setSectionHistory(history=>history.slice(0,-1));
       setActivePatientSection(next);
       custom.detail.handled=true;
     }else if(sectionOpen){
       setSectionOpen(false);
       setSectionHistory([activePatientSection]);
       custom.detail.handled=true;
     }
   };
   window.addEventListener('cardiovault-patient-back',onBack);
   return()=>window.removeEventListener('cardiovault-patient-back',onBack);
 ...
 const openSection=(id:typeof activePatientSection)=>{
   setActivePatientSection(id);
   setSectionHistory(history=>history[history.length-1]===id?history:[...history,id]);
   setSectionOpen(true);
 };
