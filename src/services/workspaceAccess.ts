...
export async function redeemUnitAccessCode(raw:string):Promise<WorkspaceAccessState>{
  ...
  const membership={uid:id,workspaceId:MASTER_WORKSPACE_ID,unitId:access.unitId,role:access.role==='view_only'?'view_only':'clinical_editor',accessCodeHash,active:true,forceReauth:false,joinedAt:new Date().toISOString(),updatedAt:new Date().toISOString()};
  if(Capacitor.isNativePlatform()){
...
export async function validateCurrentWorkspaceAccess():Promise<boolean>{const id=await uid();if(!id)return false;if(await isMasterAccount())return !!(await ensureOwnerWorkspace());try{let membership:any=null;if(Capacitor.isNativePlatform()){const result:any=await FirebaseFirestore.getDocument({reference:`workspaces/${MASTER_WORKSPACE_ID}/members/${id}`});membership=safe(result?.snapshot);}else{const result=await webGetDoc(webDoc(`workspaces/${MASTER_WORKSPACE_ID}/members/${id}`));membership=result.exists()?result.data():null;}if(!membership?.active||membership?.forceReauth||!membership?.accessCodeHash)return false;let access:any=null;if(Capacitor.isNativePlatform()){const result:any=await FirebaseFirestore.getDocument({reference:`accessCodes/${membership.accessCodeHash}`});access=safe(result?.snapshot);}else{const result=await webGetDoc(webDoc(`accessCodes/${membership.accessCodeHash}`));access=result.exists()?result.data():null;}if(!access?.active||access.workspaceId!==MASTER_WORKSPACE_ID)return false;return true;}catch(error){console.warn('Workspace access validation failed:',error);return false;}}
