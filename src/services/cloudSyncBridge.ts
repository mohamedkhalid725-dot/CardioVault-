...
async function migrateAllLegacyMembersIntoWorkspace(workspaceId:string):Promise<number>{
  const members=await getCollectionDocuments(workspacePath(workspaceId,'members'));
  let migrated=0;
  for(const member of members){
    const uid=String(member.data?.uid||member.id||'');
    const unitId=String(member.data?.unitId||'');
    const role=member.data?.role==='view_only'?'view_only':'clinical_editor';
    ...
    try{
      migrated+=await migrateLegacyMemberPatients(uid,{
        ...
        role,
        unitId,
        unitName:String(member.data?.unitName||'')
      });
    }catch(error){
