...
/** CardioVault Cloud Sync v8 — account-isolated, non-destructive Firestore sync. */
let installed = false;
let syncing = false;
...
const ensureFirestoreNetwork = async () => {
  if (Capacitor.isNativePlatform()) await FirebaseFirestore.enableNetwork();
};
...
/** Firestore-safe recursive serializer for the Capacitor bridge. */
...
async function syncCollection(uid: string, collection: string, records: any[]): Promise<void> {
  ...
  try {
    ...
    // A cleanup/read problem must not turn a successful upload into a failed sync.
...
async function syncLocalDatabase(uid: string): Promise<void> {
  ...
  if (!verifiedUid || verifiedUid !== uid) throw new Error('Firebase native authentication session is missing or changed. Sign in with Google again.');
  ...
  try {
    ...
    if (!verifiedAgain || verifiedAgain !== uid) throw new Error('Firebase authentication session expired before cloud write.');
...
/** Load cloud data only after a successful Firestore read. Never erase local data on a network/rules error. */
export async function loadCurrentUserFromCloud(): Promise<{ uid: string; found: boolean } | null> {
  ...
  try {
    ...
    const found = !!(safeUnits.length || safeBeds.length || safePatients.length);
    if (!found) return { uid, found: false };
    ...
    recordCloudError(error);
    // Keep the existing local clinical workspace intact if cloud read fails.
    console.warn('Cloud database read failed; local workspace was preserved.', error);
...
export async function syncCurrentUserNow(): Promise<boolean> {
  ...
  const uid = await currentUid();
  if (!uid) {
    const detail = 'No native Firebase user is signed in. Use Sign in with Google so the Android Firebase session is established.';
    recordCloudError(new Error(detail));
    return false;
  }
  setActiveUid(uid);
  try {
